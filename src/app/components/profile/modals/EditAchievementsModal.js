...
export default function EditAchievementsModal({
  ...
  const validate = () => {
    ...
    achievements.forEach((ach, idx) => {
      ...
      if (!ach.date.trim()) newErrors[`${idx}-date`] = "Date is required";
          ...
          >
            ...
            <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-4 flex justify-between items-center text-white flex-shrink-0">
              <h2 className="text-lg font-bold flex items-center gap-2">
                <Award className="w-5 h-5" /> Edit Honors & Awards
              </h2>
              <button
                onClick={onClose}
            ...
            >
              ...
              <div className="p-[2px] bg-gradient-to-tr from-blue-600 via-violet-500 to-purple-600 rounded-xl mb-6">
                <div className={`p-4 rounded-[calc(0.75rem-2px)] flex items-start gap-3 ${darkMode ? "bg-[#121213] text-blue-300" : "bg-blue-50 text-blue-800"}`}>
                  ...
                  <div className="text-sm leading-relaxed space-y-1">
                    <p className="font-black uppercase tracking-wide">Earn Points for Awards!</p>
                    <p>Add up to <span className="font-black">3 awards with a valid Link OR Proof Image</span> to earn <span className="font-black">15 points each</span> (maximum <span className="font-black">45 points</span> total).</p>
                    ...
                    <p className={`text-[10px] font-medium mt-1 ${darkMode ? "text-slate-400" : "text-gray-500"}`}>
                      Faculty & Admin can always see proof • Toggle public visibility per entry.
                    </p>
                  </div>
                </div>
              </div>
                      ...
                      <div className="p-5 sm:p-8 space-y-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                          ...
                          <div className="space-y-1.5">
                            ...
                            <div className={`p-[2px] bg-gradient-to-tr from-teal-500 to-emerald-500 rounded-xl ${errors[`${index}-date`] ? "from-red-500 to-red-600" : ""}`}>
                              <input
                                type="text"
                                value={ach.date}
                        ...
                        <div className="space-y-1.5">
                          <label className={`text-xs font-black uppercase tracking-widest flex items-center gap-1.5 ${darkMode ? "text-pink-400" : "text-pink-600"}`}>
                            <LinkIcon className="w-3.5 h-3.5" /> Proof Link
                        ...
                        {/* Proof Image Upload */}
                        <div className="space-y-1.5 pt-4 border-t border-dashed border-gray-200 dark:border-white/10">
                          <label className={`text-xs font-black uppercase tracking-widest flex items-center gap-1.5 ${darkMode ? "text-orange-400" : "text-orange-600"}`}>
                            <ImageIcon className="w-3.5 h-3.5" /> Proof Image
                            <span className={`text-[10px] font-medium normal-case ${darkMode ? "text-slate-400" : "text-gray-500"}`}>(Optional certificate or photo)</span>
                          </label>
                          {!ach.proofImage ? (
              ...
              >
                ...
                <span className="font-bold">Add Honor / Award</span>
              </button>
            </div>
